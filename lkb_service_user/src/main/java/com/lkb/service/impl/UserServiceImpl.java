...
public class UserServiceImpl implements UserService {
    ...
    public void sendSms(String phone) {
        ...
        int code = random.nextInt(999999);//生成验证码
        if (code < 100000) {//位数处理
            code = code + 100000;
        }
        ...
        rabbitTemplate.convertAndSend("","queue.sms", JSON.toJSONString(map));//以对象信息保存在mq
    }

@Reference
private com.lkb.service.teammanager.UserService userService;
    ...
    public void add(User user, String smsCode) {
        ...
        if(user.getUsername()==null){
            user.setUsername(user.getPhone());//把手机号做为用户名
        }
        //效验用户名是否已经注册
        User searchUser=new User();
        searchUser.setUsername( user.getUsername() );
        System.out.println(user.getUsername());
        System.out.println(userMapper.selectCount(searchUser));
        ...
        Map mapTeam=new HashMap();
        mapTeam.put("username",user.getUsername());
        mapTeam.put("password",user.getPassword());
        com.lkb.pojo.teammanager.User userTeam=new com.lkb.pojo.teammanager.User();
        try {
            userService.insertNewTeam(mapTeam);
