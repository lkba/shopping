...
import com.lkb.dao.CategoryMapper;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void add(Category category) {
        ...
        saveCategoryTreeToRedis();//数据修改好重新缓存
    }
...
//        return findByParentId(categories,0);
    ...
    public List<Map> findCategory3List() {
        List<Map> category3s = new LinkedList<>();
        List<Map> mapList = (List<Map>) redisTemplate.boundValueOps(CacheKey.CATEGORY_TREE).get();
        for (Map map : mapList) {
            for (Map category3List : (List<Map>) map.get("menus")) {
                for (Map category3 : (List<Map>) category3List.get("menus")) {
                    category3s.add(category3);
                }
            }
        }
        return category3s;
    ...
    private List<Map> findByParentId(List<Category> categoryList, Integer parentId) {
        ...
        for (Category category : categoryList) {
            if (category.getParentId().equals(parentId)) {//只取没有父级的导航做为一个Map
                Map map = new HashMap();
